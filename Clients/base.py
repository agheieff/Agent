...
class BaseLLMClient(ABC):
    ...
    def extract_response_content(self, message) -> str:
        try:
            ...
            # Handle content attribute directly
    ...
    async def get_response(self, p: Optional[str], s: Optional[str], h: List[Dict] = None, temperature: float = 0.5, max_tokens: int = 4096, tool_usage: bool = False, model: Optional[str] = None) -> Optional[str]:
        try:
            ...
            # Check context length and adjust max_tokens if needed
            context_window = mi.context_window if mi else 128000
            
            # Set a reasonable default max completion length
            # Most models have a max completion length lower than their context window
            # Use 8192 as a safe default for most modern models if not specified
            if max_tokens >= context_window / 2:
                max_tokens = min(8192, context_window // 4)  # Use smaller of 8192 or 25% of context window
