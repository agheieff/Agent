...
class AutonomousAgent:
    def __init__(self,api_key:str,model:str="default-model",provider:str="openai",test_mode:bool=False,config:Dict[str,Any]=None):
        ...
        self.agent_state={"started_at":datetime.now().isoformat(),"last_active":datetime.now().isoformat(),"tools_executed":0,"tasks_completed":0,"status":"initializing","last_error":None,"current_task":None}
    ...
    async def run(self,initial_prompt:str,system_prompt:str=""):
        try:
            ...
            self.local_conversation_history=[{"role":"system","content":system_prompt},{"role":"user","content":initial_prompt}]
    ...
    async def _generate_response(self,system_prompt:Optional[str],user_input:str)->Union[str,Dict[str,Any]]:
        try:
            if system_prompt is not None:
                self.local_conversation_history=[{"role":"system","content":system_prompt},{"role":"user","content":user_input}]
            else:
    ...
    async def _process_response(self,response)->bool:
        try:
            ...
            if a and output_manager:
                ...
                print(f"\n[Agent Answer]: {a}\n")
            tc=d.get("tool_calls",[])
            if not tc and"action"in d:
                ac=d["action"]
                ai=d.get("action_input",{})or d.get("parameters",{})
                if ac:
                    tc=[{"name":ac,"params":ai}]
            if tc:
                rs=await self.tool_manager.process_message_from_calls(tc,output_format=self.default_output_format)
                if rs:
                    self.local_conversation_history.append({"role":"user","content":rs})
                    for c in tc:
                        if isinstance(c,dict)and c.get("name")=="finish":
